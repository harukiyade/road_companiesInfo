/*
  タイプIの修正・統合処理スクリプト（132.csv）
  
  - 企業名+住所などで同じ企業を特定して1つに統合
  - 種別・状態・NDA締結・AD締結・担当者ヘッダーは無視
  - フィールドマッピングを修正
...
async function main() {
  ...
  console.log("📊 タイプIの修正・統合処理を開始します\n");
  ...
  try {
    ...
    console.log(`📄 ${path.basename(TYPE_I_FILE)}: ${records.length} 行\n`);
    ...
    records.forEach((row, idx) => {
      ...
      for (const [header, value] of Object.entries(row)) {
        ...
        // 数値フィールドの処理
        if (["capitalStock", "employeeCount", "officeCount", "factoryCount", "storeCount",
             "revenue1", "revenue2", "revenue3", "revenue4", "revenue5",
             "profit1", "profit2", "profit3", "profit4", "profit5"].includes(mappedField)) {
  ...
  console.log(`📊 総レコード数: ${allCompanies.length}\n`);
