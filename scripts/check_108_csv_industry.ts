/* 
  108.csvからインポートしたデータの業種フィールドを確認するスクリプト
  
  使い方:
    GOOGLE_APPLICATION_CREDENTIALS=/path/to/serviceAccountKey.json \
    npx ts-node scripts/check_108_csv_industry.ts
*/
...
// メイン処理
async function main() {
  console.log("📄 108.csvからインポートしたデータの業種フィールドを確認します\n");
  ...
  for (let i = 0; i < checkCount; i++) {
    ...
    try {
      ...
      if (!snap.empty) {
        ...
        const csvIndustry1 = row["業種1"]?.trim() || "";
        const csvIndustry2 = row["業種2"]?.trim() || "";
        const csvIndustry3 = row["業種3"]?.trim() || "";

        console.log(`\n${"=".repeat(80)}`);
        console.log(`[行${rowNumber}] ${companyName}`);
        console.log(`docId: ${doc.id}`);
        console.log(`\nCSV:`);
        console.log(`  業種1: ${csvIndustry1}`);
        console.log(`  業種2: ${csvIndustry2}`);
        console.log(`  業種3: ${csvIndustry3}`);
        console.log(`\nFirestore:`);
        ...
        // 業種が正しくマッピングされているか確認
        let allCorrect = true;
        if (csvIndustry1 && data.industryLarge !== csvIndustry1) {
          console.log(`  ❌ industryLargeが一致しません`);
          allCorrect = false;
        }
        if (csvIndustry2 && data.industryMiddle !== csvIndustry2) {
          console.log(`  ❌ industryMiddleが一致しません`);
          allCorrect = false;
        }
        if (csvIndustry3 && data.industrySmall !== csvIndustry3) {
          console.log(`  ❌ industrySmallが一致しません`);
          allCorrect = false;
        }
        
        if (allCorrect && (csvIndustry1 || csvIndustry2 || csvIndustry3)) {
          console.log(`  ✅ 業種は正しくマッピングされています`);
        }
      } else {
        console.log(`\n${"=".repeat(80)}`);
