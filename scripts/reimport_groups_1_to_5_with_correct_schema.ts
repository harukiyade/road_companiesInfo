/* 
  グループ1-5のCSVファイルを指定された155フィールド構造に合わせて再インポートするスクリプト
  ...
  使い方:
    ...
    npx ts-node scripts/reimport_groups_1_to_5_with_correct_schema.ts
*/
...
// 代表者名から生年月日を除去（生年月日が含まれている場合）
function cleanRepresentativeName(name: string | null | undefined): string | null {
  if (!name || typeof name !== "string") return null;
  
  let cleaned = name.trim();
  
  // 生年月日パターンを除去（例: 6/20/73, 3/12/60, 12/22/69）
  cleaned = cleaned.replace(/\d{1,2}\/\d{1,2}\/\d{2,4}/g, "").trim();
  cleaned = cleaned.replace(/\d{4}[\/年-]\d{1,2}[\/月-]\d{1,2}/g, "").trim();
...
function buildHeaderToFieldMap(headers: string[]): Record<string, string> {
  ...
  const fieldMappings: Array<{ target: string; aliases: string[] }> = [
    ...
    { target: "representativePostalCode", aliases: ["代表者郵便番号"] },
    { target: "representativeHomeAddress", aliases: ["代表者住所"] },
    { target: "representativeBirthDate", aliases: ["代表者誕生日"] }, // 重要: 代表者誕生日を正しくマッピング
...
function mapRowToCompanyFields(
  ...
  for (const [header, value] of Object.entries(row)) {
    ...
    // 特別な処理が必要なフィールド
    if (targetField === "representativeName") {
      // 代表者名から生年月日を除去
      const cleanedName = cleanRepresentativeName(value);
      if (cleanedName) {
        result.representativeName = cleanedName;
      }
    } else if (targetField === "representativeBirthDate") {
      // 代表者誕生日を正しく設定
      const birthDate = extractBirthDate(value);
      if (birthDate) {
        result.representativeBirthDate = birthDate;
      }
    } else if (targetField === "established") {
...
// ==============================
async function main() {
  console.log("🔄 グループ1-5のCSVファイルを指定された155フィールド構造に合わせて再インポートします...\n");
