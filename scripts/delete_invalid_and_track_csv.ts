/* 
  問題のある会社名のドキュメントを削除し、どのCSVから来たかを記録するスクリプト

  使い方:
    GOOGLE_APPLICATION_CREDENTIALS=/path/to/serviceAccountKey.json \
    npx ts-node scripts/delete_invalid_and_track_csv.ts <report.json> [csv-dir]
*/
...
async function deleteInvalidCompanies(
  ...
  for (const company of targetCompanies) {
    ...
    if (processedCount % 100 === 0) {
      console.log(`  処理中: ${processedCount}/${targetCompanies.length}件 (削除済み: ${deletedCount}件)`);
    }

    // CSVファイルで検索
    const csvMatches = findCompanyInCsv(csvDir, company.corporateNumber);
...
async function main() {
  ...
  if (!reportPath) {
    ...
    console.error('');
    console.error('使用方法:');
    console.error('  npx ts-node scripts/delete_invalid_and_track_csv.ts <report.json> [csv-dir]');
    process.exit(1);
  }
  ...
  // 削除実行
  const result = await deleteInvalidCompanies(db, report.companies, csvDir);
