...
def normalize_corp_num(val):
    ...
    s = re.sub(r"[^\d]", "", s)
    # 指数表記で丸められた可能性があるため、10桁以上あれば一旦通す
...
def process_file(conn, file_path):
    ...
    with open(file_path, "r", encoding=enc, errors="replace") as f:
        ...
        batch = []
        for row in reader:
            data = {v: None for v in HEADER_MAP.values()}
...
def upsert_batch(conn, batch):
    ...
    # 1. 既存レコードのID特定 (法人番号優先、次に名前+都道府県)
    for row in batch:
        row['id'] = None
    ...
    # 3. 更新実行 (transaction_typeは上書き、他はNULL補完)
    if updates:
        cols = [c for c in HEADER_MAP.values() if c != 'corporate_number']
        set_clauses = [f"{c} = COALESCE(companies.{c}, EXCLUDED.{c})" for c in cols if c != 'transaction_type']
        set_clauses.append("transaction_type = EXCLUDED.transaction_type") # 強制上書き
        set_clauses.append("updated_at = NOW()")

        query = f"""
            INSERT INTO companies (id, {', '.join(cols)})
            VALUES %s
            ON CONFLICT (id) DO UPDATE SET {', '.join(set_clauses)}
        """
        vals = [[r['id']] + [r[c] for c in cols] for r in updates]
        execute_values(cur, query, vals)

    # 4. 新規登録実行 (譲渡企業などがDBにない場合)
    if inserts:
        cols = list(set(HEADER_MAP.values()))
        query = f"INSERT INTO companies ({', '.join(cols)}) VALUES %s ON CONFLICT DO NOTHING"
        vals = [[r[c] for c in cols] for r in inserts]
        execute_values(cur, query, vals)
