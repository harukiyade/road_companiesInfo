/*
  ...
  - 取引種別・SBフラグ・NDA・AD・ステータス・備考ヘッダーは無視
  - フィールドマッピングを修正
...
const TYPE_E_FILES = [
  ...
  "csv/118.csv", "csv/120.csv", "csv/121.csv", "csv/122.csv",
  "csv/123.csv", "csv/124.csv", "csv/125.csv"
];

// 無視するフィールド
const IGNORED_FIELDS = new Set([
  "取引種別", "SBフラグ", "NDA", "AD", "ステータス", "備考",
  "ID", "会社ID", "取引状態"
]);
...
async function main() {
  ...
  console.log("📊 タイプEの修正・統合処理を開始します\n");

  const allCompanies: CompanyData[] = [];

  // CSVファイルを読み込み
  for (const file of TYPE_E_FILES) {
    ...
    try {
      ...
      records.forEach((row, idx) => {
        ...
        for (const [header, value] of Object.entries(row)) {
          ...
          // 数値フィールドの処理
          if (["capitalStock", "employeeCount", "latestRevenue", "latestProfit"].includes(mappedField)) {
