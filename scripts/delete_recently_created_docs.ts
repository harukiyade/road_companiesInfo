/* 
  最近作成されたドキュメントを削除するスクリプト
  
  使い方:
    GOOGLE_APPLICATION_CREDENTIALS=./albert-ma-firebase-adminsdk-iat1k-a64039899f.json \
    npx ts-node scripts/delete_recently_created_docs.ts [ログファイル名]
*/
...
// メイン処理
async function main() {
  const logFileName = process.argv[2] || "created_docs_first_5_rows_1765462249873.txt";
  ...
  for (const line of lines) {
    // 形式: "38.csv - 行1: QPGnRWzl8XBlkKDFJH6H (丹羽興業株式会社)"
