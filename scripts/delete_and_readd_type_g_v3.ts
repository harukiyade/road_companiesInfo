/* 
  タイプG（127.csv）を削除して、業種自動分類機能付きで再作成
  
  業種名（例：「水産・水産加工品」）から大分類〜細分類を自動推測
*/
...
const TYPE_G_DOC_ID = "Z33cLT0Th8KyHEkQuKsh";
...
function inferIndustryCategories(industryName: string | null): {
  ...
  if (industry.includes("水産") || industry.includes("水産加工")) {
    return {
      ...
      industryDetail: industry, // 元の業種名を細分類に
    };
  }
...
/**
 * タイプG（127.csv）のデータをマッピング（業種自動分類対応版）
 */
...
async function main() {
  console.log("================================================================================");
  console.log("タイプG: 削除と再追加（業種自動分類対応版）");
  console.log("================================================================================");
  console.log();
  ...
  // 3. 業種分類を自動推測
  console.log("【STEP 3】業種分類を自動推測中...");
  const industryCategories = inferIndustryCategories(row["業種"]);
  console.log(`  元の業種名: ${row["業種"]}`);
  console.log(`  推測された大分類: ${industryCategories.industryLarge}`);
  console.log(`  推測された中分類: ${industryCategories.industryMiddle}`);
  console.log(`  推測された小分類: ${industryCategories.industrySmall}`);
  console.log(`  細分類: ${industryCategories.industryDetail}`);

  console.log();

  // 4. データをマッピング
  console.log("【STEP 4】データをマッピング中...");
  ...
  // 5. Firestoreに新規追加
  console.log("【STEP 5】Firestoreに追加中...");
  ...
  // 6. 確認
  console.log("【STEP 6】追加したデータを確認中...");
  ...
  if (newData) {
    ...
    console.log(`  業種（元の値）: ${newData.industry}`);
    ...
    if (newData.industryLarge && newData.industryMiddle && newData.industrySmall) {
      console.log("✅ 業種大分類〜細分類が自動で入力されました！");
    } else {
      console.log("⚠️  一部の業種分類が推測できませんでした");
    }
    
    console.log("\n🎉 タイプG（業種自動分類対応版）の作成が完了しました！");
  }
