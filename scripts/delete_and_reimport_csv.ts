/* 
  指定したCSVファイルの内容と一致するドキュメントを削除してから再インポートするスクリプト
  
  使い方:
    DRY_RUN=1 npx tsx scripts/delete_and_reimport_csv.ts 53.csv  # 削除せず候補だけログ
    npx tsx scripts/delete_and_reimport_csv.ts 53.csv             # 実際に削除して再インポート
*/
...
async function main() {
  ...
  if (!csvFileName) {
    console.error("使用方法: npx tsx scripts/delete_and_reimport_csv.ts <csvFileName>");
    console.error("例: npx tsx scripts/delete_and_reimport_csv.ts 53.csv");
    process.exit(1);
  }
  ...
  if (DRY_RUN) {
    ...
    console.log(`⚠️  本番モード: ${csvFileName} の内容と一致するドキュメントを削除します\n`);
  }
  ...
  if (deleteCandidates.length === 0) {
    console.log(`✅ ${csvFileName} の内容と一致するドキュメントは見つかりませんでした`);
    console.log(`\n📥 ${csvFileName} を再インポートします...`);
    if (!DRY_RUN) {
      const { execSync } = require("child_process");
      execSync(`npx tsx scripts/automation/import_csv_companies_generic.ts ./csv/${csvFileName}`, {
        stdio: "inherit",
        cwd: process.cwd(),
      });
    }
