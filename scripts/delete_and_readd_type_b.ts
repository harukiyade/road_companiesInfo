...
const TYPE_B_DOC_ID = "nWaQxIQIZEQUK9Bk49V4";
const CSV_PATH = "csv/12.csv";
...
function getEmptyTemplate(): Record<string, any> {
  return {
    ...
    representativePostalCode: null, // 代表者郵便番号
...
function mapTypeB(row: Record<string, any>): Record<string, any> {
  ...
  data.representativePostalCode = row["代表者郵便番号"] || null; // 代表者郵便番号
  data.representativeRegisteredAddress = row["代表者住所"] || null; // 代表者住所
  ...
  data.foundingYear = null; // タイプBは創業年フィールドあるが、このCSVにはデータなし
...
async function main() {
  ...
  try {
    ...
    if (docSnap.exists) {
      ...
      console.log(`  [修正前] representativePostalCode: ${oldData?.representativePostalCode || "(フィールドなし)"}`);
      console.log(`  [修正前] representativeRegisteredAddress: ${oldData?.representativeRegisteredAddress}`);
      console.log(`  [修正前] representativeHomeAddress: ${oldData?.representativeHomeAddress}`);
      
      await docRef.delete();
      console.log(`✓ 削除完了 (ID: ${TYPE_B_DOC_ID})`);
    } else {
      console.log(`⚠️  ドキュメントが見つかりません (ID: ${TYPE_B_DOC_ID})`);
    }
  } catch (error: any) {
  ...
  console.log(`✓ マッピング完了`);
  console.log(`  [修正後] representativePostalCode: ${companyData.representativePostalCode}`);
  console.log(`  [修正後] representativeRegisteredAddress: ${companyData.representativeRegisteredAddress}`);
  console.log(`  [修正後] representativeHomeAddress: ${companyData.representativeHomeAddress}`);
  ...
  if (newData) {
    ...
    console.log(`  株主: ${newData.shareholders}`);
