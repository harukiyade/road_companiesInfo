...
使い方:
  POSTGRES_PASSWORD=xxx python scripts/audit_csv_vs_db_integrity.py [--report report.csv]
"""
...
def resolve_company_id(row_data, cache_corp, cache_name_pref, prefectures):
    """1行分のデータから companies.id を解決（法人番号優先、次に名前+都道府県）"""
...
def load_id_caches(conn):
    ...
    cur.close()
    return cache_corp, cache_name_pref
...
def run_audit(conn, csv_files, report_rows, cache_corp, cache_name_pref, db_columns_set):
        ...
        try:
            with open(fp, "r", encoding=enc, errors="replace") as f:
                ...
                for row in reader:
                    ...
                    company_id = resolve_company_id(row_data, cache_corp, cache_name_pref, PREFECTURES)
...
def main():
    ...
    try:
        ...
        print(f"DBキャッシュ: 法人番号 {len(cache_corp)} 件, 名前+都道府県 {len(cache_name_pref)} 件")

        report_rows = []
        run_audit(conn, csv_files, report_rows, cache_corp, cache_name_pref, db_columns_set)
