# 整合性レポート一式 (full_integrity_20260322_043300)

- 生成日時: 2026-03-22T04:33:06
- 対象: `fixed_csv_3` の `unit_million` / `unit_yen` 配下の全 `*.csv`

> 試行モード: `--max-rows-per-file 3000`（ファイルあたりの最大走査行数）
...
| `05_NDA締結済_件数サマリ.tsv` | （従来互換）NDA=締結済 件数 |
| `06_csv_inventory.tsv` | （従来互換）全CSV行数・エンコーディング |

## サマリ（件数）

- カテゴリ1 列数不一致行: **0**
- カテゴリ1 列ズレ強シグナル行: **3180**
- カテゴリ2 日付異常セル: **82105**
- カテゴリ3 重複ヘッダファイル: **26**
- カテゴリ3 import_firstTime J列非空（行集計）: **121367**
- カテゴリ3 住所異常行: **4487**
- カテゴリ3 財務単位メモ行: **11894**
- カテゴリ3 株主/仕入れ先ギャップ行: **31**
- カテゴリ5 重複会社名グループ: **48605**
- NDA締結済（合算）: **38815**
- CSVファイル数: **182**
